...
class Ekipman:
    ...
    # Ekranlarda kullanılan, servis katmanında doldurulan alan
    sorumlu: str = ""
    ...
    def sonraki_kontrol(self) -> str:
        return utils.sonraki_kontrol_tarihi(self.son_kontrol, self.periyot)

    @property
    def kalan_gun(self) -> int:
        return utils.kalan_gun(self.sonraki_kontrol)

    @property
    def durum(self):
        """Döndürür: ("Geçerli", "check-circle", "success")"""
        return utils.durum_belirle(self.kalan_gun)
    ...
    def kalan_gun_yazisi(self) -> str:
        return utils.kalan_gun_yazisi(self.kalan_gun)
